...
import type { Product } from "@/store/ui-slice/product-slice";
import {
  ArrowUpRight,
  DollarSign,
  ShoppingBag,
...
const Analytics = () => {
  ...
  useEffect(() => {
    const calcStats = () => {
      ...
      const monthlyData = generateMonthlyData(products);

      // Calculate customer segments based on product categories
      const customerSegments = calculateCustomerSegments(products);
  ...
  const generateMonthlyData = (products: Product[]) => {
    const months = [
      "Jan",
      "Feb",
      "Mar",
    ...
    return months.map((month) => {
      ...
      return {
        ...
        customers: Math.floor(
          (totalRevenue * (0.3 + Math.random() * 0.3)) / 12
        ),
  ...
  const calculateCustomerSegments = (products: Product[]) => {
    const categories: Record<string, number> = {};
    products.forEach((product) => {
      categories[product.category] =
        (categories[product.category] || 0) + product.stock;
    });
